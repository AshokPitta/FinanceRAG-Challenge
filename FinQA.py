...
from financerag.tasks import FinQA
...
def ensure_prep_files():
    """FinanceRAG HFDataLoader defaults to *_prep.jsonl; create copies if absent."""
    root = "./dataset/FinQA"
    qp = os.path.join(root, "queries_prep.jsonl")
    cp = os.path.join(root, "corpus_prep.jsonl")
    if not os.path.exists(qp) and os.path.exists(os.path.join(root, "queries.jsonl")):
        with open(os.path.join(root, "queries.jsonl"), "r", encoding="utf-8") as f_in, \
             open(qp, "w", encoding="utf-8") as f_out:
            for line in f_in: f_out.write(line)
        print("[prep] created queries_prep.jsonl")
    if not os.path.exists(cp) and os.path.exists(os.path.join(root, "corpus.jsonl")):
        with open(os.path.join(root, "corpus.jsonl"), "r", encoding="utf-8") as f_in, \
             open(cp, "w", encoding="utf-8") as f_out:
            for line in f_in: f_out.write(line)
        print("[prep] created corpus_prep.jsonl")
...
def stringify_table(table):
    """Serialize table-like structures (dict/list/csv-ish) into a compact header+rows string."""
    try:
...
def augment_corpus_with_tables(task):
    """In-place: enrich doc text with captions/headers/rows so retrievers & CEs can match tables."""
...
def bm25_tokenize_list(texts):
    """Numeric-aware tokenization (%,$,bn,m, numbers). FinanceRAG BM25 lowercases already."""
...
def table_feature_score(q, doc_text):
    """Light features for numerical QA: number/year/header overlap + unit alignment."""
...
if __name__ == "__main__":
    ...
    print(f"Corpus:  {len(task.corpus)}")

    # 1) Table-aware augmentation
    augment_corpus_with_tables(task)

    # 2) Dual-dense retrieval (BGE + MXBAI) with finance prompts
    encoder1 = SentenceTransformerEncoder(
        model_name_or_path="BAAI/bge-large-en-v1.5",
        query_prompt="Represent this financial question to retrieve exact rows/figures from reports, preserving units and fiscal periods: ",
        doc_prompt="Represent this table/paragraph for retrieval; include headers, units (%, $, bn, m), and fiscal periods: ",
    )
    retriever1 = DenseRetrieval(model=encoder1)
    dense1 = task.retrieve(retriever=retriever1, top_k=150)
    ...
    dense2 = task.retrieve(retriever=retriever2, top_k=150)
    ...
    bm25 = task.retrieve(retriever=bm25_retriever, top_k=150)

    # 4) Small grid search for alpha (dense fusion) and beta (hybrid fusion)
    qrels_candidates = [
        "./dataset/FinQA/FinQA_qrels.tsv",
        os.path.join(os.getcwd(), "dataset", "FinQA", "FinQA_qrels.tsv"),
    ]
    qrels_path = next((p for p in qrels_candidates if os.path.exists(p)), None)
    if qrels_path is None:
        raise FileNotFoundError("FinQA_qrels.tsv not found in ./dataset/FinQA")
    ...
    reranked_bge = task.rerank(reranker=rer_bge, results=fused_results, top_k=100, batch_size=32)
    reranked_min = task.rerank(reranker=rer_min, results=fused_results, top_k=100, batch_size=32)
    ...
    for qid in task.queries:
        ...
        # feature on augmented text
        feat = {}
        qtext = task.queries[qid]
        for did in cand_docs:
            feat[did] = table_feature_score(qtext, task.corpus[did].get("text",""))
        feat_n = minmax(feat)

        # weights tuned for numerical/tabular QA (same recipe that worked well on ConvFinQA)
    ...
    task.rerank_results = final_results
    task.save_results(output_dir=out_dir)
    print(f"Saved results to {out_dir}/FinQA/results.csv")

    ndcg, *_ = BaseTask.evaluate(qrels=qrels, results=final_results, k_values=[10])
    print(f"FinQA -> nDCG@10: {ndcg['NDCG@10']:.5f}")
