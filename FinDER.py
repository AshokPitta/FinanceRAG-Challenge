...
# === Print dataset info ===
dataset_name = "FinDER"
num_queries = len(finder_task.queries)
num_corpus = len(finder_task.corpus)
...
dense1_results = finder_task.retrieve(retriever=retriever1, top_k=150)
...
dense2_results = finder_task.retrieve(retriever=retriever2, top_k=150)
...
bm25_model = SimpleBM25(finder_task.corpus)
bm25_retriever = BM25Retriever(model=bm25_model)
bm25_results = finder_task.retrieve(retriever=bm25_retriever, top_k=150)

# === Step 4: Score Fusion (Dual Dense + BM25, a=0.6 dense fusion) ===
...
fused_results = {}
for qid in finder_task.queries:
...
# === Step 6: Save Results ===
finder_task.rerank_results = reranked
out_dir = os.path.join(BASE_DIR, "results/FinDER/")
finder_task.save_results(output_dir=out_dir)
print(f"Saved results to {out_dir}/FinDER/results.csv")

# === Step 7: Evaluate ===
qrels_path = os.path.join(BASE_DIR, "dataset/FinDER/FinDER_qrels.tsv")
...
print(f"Dual-Dense + BM25 + BGE rerank nDCG@10: {ndcg['NDCG@10']:.4f}")
